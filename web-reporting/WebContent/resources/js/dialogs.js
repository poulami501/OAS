...
function viewBroadcastMessage() {
	...
	if (isHidden) {
		$.ajax(
		{
				...
				url:		'/ImmediateReportingWeb/immediateReportingOperation/broadcastMessage.do',
				type:		'POST',
				data:		param,
				dataType:	'html',
...
function resetErrorMessages(){
    if($("#contentMain").length>0)	$("#contentMain").text("");
	if($("#errorIcon").length>0)    $('#errorIcon').hide();
	if($("#infoIcon").length>0)    $('#infoIcon').hide();
	if($("#displayMessageMain").length>0)	document.getElementById('displayMessageMain').style.display = "none";										
}
