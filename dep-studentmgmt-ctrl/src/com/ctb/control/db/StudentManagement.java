...
{ 
   ...
   // @JdbcControl.SQL(statement = "select distinct  roster.test_roster_id as rosterId, stu.student_id as id,  stu.user_name as loginId,  stu.first_name as firstName,  stu.middle_name as middleName,  stu.last_name as lastName, \t  concat(concat(stu.last_name, ', '), concat(stu.first_name, concat(' ', stu.MIDDLE_NAME))) as studentName,  stu.gender as gender,  stu.birthdate as birthDate,  stu.grade as grade,  stu.ext_pin1 as studentIdNumber,  stu.ext_pin2 as studentIdNumber2,  stu.created_by as createdBy, tadmin.test_admin_name as testSessionName, tadmin.access_code as accessCode,tadmin.test_admin_id   as testAdminId, tadmin.item_set_id     as itemSetIDTC from  org_node_student ons,  student stu,  org_node node,  org_node_category onc,  org_node_ancestor ona,  user_role urole,  users, test_roster roster, org_node_test_catalog ontcat,  item_set iset,  test_admin tadmin,  test_admin_user_role tar, product prod,  student_item_set_status sis,  test_catalog cat where  ons.student_id = stu.student_id \t and ons.activation_status= 'AC' \t and stu.activation_status= 'AC' \t and ons.org_node_id = node.org_node_id  and ons.org_node_id = ona.org_node_id  \t and ona.ANCESTOR_ORG_NODE_ID = urole.org_node_id \t and urole.activation_status = 'AC' \t and users.user_id = urole.user_id \t and users.user_name = {userName} \t and onc.org_node_category_id = node.org_node_category_id   and ontcat.org_node_id = ons.org_node_id  and iset.item_set_id = ontcat.item_set_id  and iset.item_set_type = 'TC' and tar.user_id = users.user_id  and tadmin.test_admin_id = tar.test_admin_id   and roster.test_admin_id = tadmin.test_admin_id and stu.student_id = roster.student_id and   tadmin.product_id = prod.product_id and prod.product_id = cat.product_id   and cat.test_catalog_id = {catalogId} and roster.test_completion_status in ('CO','IS')    and sis.test_roster_id = roster.test_roster_id  and sis.validation_status = 'VA' and roster.validation_status in ('VA', 'PI') {sql: searchCriteria}",
	@JdbcControl.SQL(statement = "select distinct roster.test_roster_id as rosterId, stu.student_id as id, stu.user_name as loginId, stu.first_name as firstName, stu.middle_name as middleName, stu.last_name as lastName, concat(concat(stu.last_name, ', '), concat(stu.first_name, concat(' ', stu.MIDDLE_NAME))) as studentName, stu.gender as gender, stu.birthdate as birthDate, stu.grade as grade, stu.ext_pin1 as studentIdNumber, stu.ext_pin2 as studentIdNumber2, stu.created_by as createdBy, tadmin.test_admin_name as testSessionName, tadmin.access_code as accessCode, tadmin.test_admin_id as testAdminId, tadmin.item_set_id as itemSetIDTC, (select count(1)  from  item_set_item ist, item item,student_item_set_status siss where item_set_type = 'TD' and ist.item_set_id = siss.item_set_id and item.item_id = ist.item_id and item.item_type = 'CR' and siss.test_roster_id = roster.test_roster_id and (item.answer_area is null or item.answer_area = 'AudioItem')) as itemCountCRAI  from org_node_student ons, student stu, org_node node, org_node_category onc, org_node_ancestor ona, org_node_ancestor ona1, user_role urole, users, test_roster roster, org_node_test_catalog ontcat, item_set iset, test_admin tadmin, student_item_set_status sis, test_catalog cat where ons.student_id = stu.student_id and ons.activation_status = 'AC' and stu.activation_status = 'AC' and ons.org_node_id = node.org_node_id and ons.org_node_id = ona.org_node_id and ona1.ANCESTOR_ORG_NODE_ID = urole.org_node_id and ona.ANCESTOR_ORG_NODE_ID = urole.org_node_id and urole.activation_status = 'AC' and users.user_id = urole.user_id and users.user_name = {userName} and onc.org_node_category_id = node.org_node_category_id and ontcat.org_node_id = ons.org_node_id and iset.item_set_id = ontcat.item_set_id and iset.item_set_type = 'TC' and ona1.org_node_id = tadmin.creator_org_node_id and roster.test_admin_id = tadmin.test_admin_id and stu.student_id = roster.student_id and tadmin.test_catalog_id = cat.test_catalog_id and cat.test_catalog_id = {catalogId} and roster.test_completion_status in ('CO', 'IS') and sis.test_roster_id = roster.test_roster_id and sis.validation_status = 'VA' and roster.validation_status in ('VA', 'PI') {sql: searchCriteria} union select distinct roster.test_roster_id as rosterId, stu.student_id as id, stu.user_name as loginId, stu.first_name as firstName, stu.middle_name as middleName, stu.last_name as lastName, concat(concat(stu.last_name, ', '), concat(stu.first_name, concat(' ', stu.MIDDLE_NAME))) as studentName, stu.gender as gender, stu.birthdate as birthDate, stu.grade as grade, stu.ext_pin1 as studentIdNumber, stu.ext_pin2 as studentIdNumber2, stu.created_by as createdBy, tadmin.test_admin_name as testSessionName, tadmin.access_code as accessCode, tadmin.test_admin_id as testAdminId, tadmin.item_set_id as itemSetIDTC, (select count(1)  from  item_set_item ist, item item,student_item_set_status siss where item_set_type = 'TD' and ist.item_set_id = siss.item_set_id and item.item_id = ist.item_id and item.item_type = 'CR' and siss.test_roster_id = roster.test_roster_id and (item.answer_area is null or item.answer_area = 'AudioItem')) as itemCountCRAI  from org_node_student ons, student stu, org_node node, org_node_category onc, org_node_ancestor ona, test_admin_user_role taur, user_role urole, users users, test_roster roster, org_node_test_catalog ontcat, item_set iset, test_admin tadmin, student_item_set_status sis, test_catalog cat where ons.student_id = stu.student_id and ons.activation_status = 'AC' and stu.activation_status = 'AC' and ons.org_node_id = node.org_node_id and ons.org_node_id = ona.org_node_id and ona.ANCESTOR_ORG_NODE_ID = urole.org_node_id and urole.activation_status = 'AC' and urole.user_id=taur.user_id and taur.user_id=users.user_id and users.user_name = {userName} and onc.org_node_category_id = node.org_node_category_id and ontcat.org_node_id = ons.org_node_id and iset.item_set_id = ontcat.item_set_id and iset.item_set_type = 'TC' and roster.test_admin_id = tadmin.test_admin_id and stu.student_id = roster.student_id and tadmin.test_catalog_id = cat.test_catalog_id and cat.test_catalog_id = {catalogId} and roster.test_completion_status in ('CO', 'IS') and sis.test_roster_id = roster.test_roster_id and sis.validation_status = 'VA' and roster.validation_status in ('VA', 'PI') {sql: searchCriteria}",
