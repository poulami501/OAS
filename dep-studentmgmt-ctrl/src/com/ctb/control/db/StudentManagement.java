...
{ 
    ...
    @JdbcControl.SQL(statement = "select distinct roster.test_roster_id as rosterId, stu.student_id as id, stu.user_name as loginId, stu.first_name as firstName, stu.middle_name as middleName, stu.last_name as lastName, concat(concat(stu.last_name, ', '),        concat(stu.first_name, concat(' ', stu.MIDDLE_NAME))) as studentName, stu.gender as gender, stu.birthdate as birthDate, stu.grade as grade, stu.ext_pin1 as studentIdNumber, stu.ext_pin2 as studentIdNumber2, stu.created_by as createdBy, tadmin.test_admin_name as testSessionName, tadmin.access_code as accessCode, tadmin.test_admin_id as testAdminId, tadmin.item_set_id as itemSetIDTC, (select count(1)    from  item_set_item ist, item item,student_item_set_status siss   where  ist.item_set_id = siss.item_set_id    and item.item_id = ist.item_id    and item.item_type = 'CR'     and siss.test_roster_id = roster.test_roster_id   and siss.completion_status in ('CO', 'IS','IC') and   siss.validation_status = 'VA'  and (item.answer_area is null or     item.answer_area = 'AudioItem')) as itemCountCRAI from org_node_student ons,   student stu,  org_node node,  org_node_category onc,  org_node_ancestor ona,  org_node_ancestor ona1,  user_role urole,  users,  test_roster roster,  org_node_test_catalog ontcat,  item_set iset,  test_admin tadmin, test_catalog cat, student_item_set_status siss where ons.student_id = stu.student_id    and ons.activation_status = 'AC'    and stu.activation_status = 'AC'    and ons.org_node_id = node.org_node_id    and ons.org_node_id = ona.org_node_id    and ona1.ANCESTOR_ORG_NODE_ID = urole.org_node_id    and ona.ANCESTOR_ORG_NODE_ID = urole.org_node_id    and urole.activation_status = 'AC'    and users.user_id = urole.user_id    and users.user_name = {userName}    and onc.org_node_category_id = node.org_node_category_id    and ontcat.org_node_id = ons.org_node_id    and iset.item_set_id = ontcat.item_set_id    and iset.item_set_type = 'TC'    and ona1.org_node_id = tadmin.creator_org_node_id    and roster.test_admin_id = tadmin.test_admin_id    and stu.student_id = roster.student_id    and roster.test_roster_id = siss.test_roster_id    and tadmin.test_catalog_id = cat.test_catalog_id    and cat.test_catalog_id = {catalogId}    and siss.completion_status in ('CO', 'IS', 'IC')     and roster.validation_status in ('VA', 'PI') {sql: searchCriteria}  union select  distinct roster.test_roster_id as rosterId, stu.student_id as id,  stu.user_name as loginId, stu.first_name as firstName, stu.middle_name as middleName,                 stu.last_name as lastName, concat(concat(stu.last_name, ', '),  concat(stu.first_name, concat(' ', stu.MIDDLE_NAME))) as studentName,  stu.gender as gender,   stu.birthdate as birthDate,   stu.grade as grade,    stu.ext_pin1 as studentIdNumber,    stu.ext_pin2 as studentIdNumber2,    stu.created_by as createdBy,    tadmin.test_admin_name as testSessionName,     tadmin.access_code as accessCode,   tadmin.test_admin_id as testAdminId,   tadmin.item_set_id as itemSetIDTC,   (select count(1)   from item_set_item   ist,   item  item,   student_item_set_status siss  where ist.item_set_id = siss.item_set_id   and item.item_id = ist.item_id and item.item_type = 'CR'   and siss.test_roster_id = roster.test_roster_id  and siss.completion_status in ('CO', 'IS','IC') and   siss.validation_status = 'VA'                   and (item.answer_area is null or          item.answer_area = 'AudioItem')) as itemCountCRAI   from org_node_student        ons,        student                 stu,          org_node_ancestor       ona,        test_admin_user_role    taur,        user_role               urole,        users                   users,        test_roster             roster,        test_admin              tadmin,    test_catalog            cat,    student_item_set_status siss  where ons.student_id = stu.student_id    and ons.activation_status = 'AC'    and stu.activation_status = 'AC'    and ons.org_node_id = ona.org_node_id    and ona.ANCESTOR_ORG_NODE_ID = urole.org_node_id    and urole.activation_status = 'AC'    and urole.user_id = users.user_id    and stu.student_id = roster.student_id    and roster.test_roster_id = siss.test_roster_id   and siss.completion_status in ('CO', 'IS', 'IC')    and roster.validation_status in ('VA', 'PI')  and roster.test_admin_id = tadmin.test_admin_id    and taur.user_id = users.user_id    and taur.test_admin_id = tadmin.test_admin_id    and tadmin.test_catalog_id = cat.test_catalog_id    and cat.test_catalog_id = {catalogId}    and users.user_name ={userName} {sql: searchCriteria}",		
    ...
    */ 
    ...
    // Change For Defect:66007
     ...
     @JdbcControl.SQL(statement = "SELECT STU.LAST_NAME || ', ' || STU.FIRST_NAME AS studentName,        to_CHAR(ROS.COMPLETION_DATE_TIME,'MM/DD/YYYY') AS completionDate,        ROS.TEST_COMPLETION_STATUS AS completionStatus,        ROS.TEST_ROSTER_ID AS testRosterId,        TA.TEST_ADMIN_NAME AS testSessionName,        PROD.PRODUCT_ID AS productId,        PROD.PRODUCT_DESCRIPTION AS productName,        ISET.ITEM_SET_ID AS itemSetId,        ISET.ITEM_SET_TYPE AS itemSetType,   ( SELECT ITM.ITEM_SET_LEVEL    FROM STUDENT_ITEM_SET_STATUS SISS, ITEM_SET ITM,ITEM_SET_PARENT     ISP   WHERE SISS.TEST_ROSTER_ID =  ROS.TEST_ROSTER_ID     AND isp.parent_item_set_id = ISET.ITEM_SET_ID     AND isp.ITEM_SET_ID = SISS.ITEM_SET_ID     AND ITM.ITEM_SET_ID = isp.ITEM_SET_ID     AND ITM.SAMPLE <> 'T'     AND ITM.ITEM_SET_LEVEL <> 'L'     AND ITM.ACTIVATION_STATUS = 'AC') AS itemSetLevel,        ISET.ITEM_SET_DISPLAY_NAME AS itemSetName,        DECODE(PROD.PRODUCT_ID, 4010, 4012, 4010) AS recommendedProductId,        (SELECT PRODUCT_NAME           FROM PRODUCT          WHERE PRODUCT_ID = DECODE(PROD.PRODUCT_ID, 4010, 4012, 4010)) AS recommendedProductName   FROM TEST_ROSTER         ROS,        PRODUCT             PROD,        STUDENT             STU,        ITEM_SET            ISET,        ITEM_SET_PARENT     ISP,        TEST_ADMIN          TA,        TEST_ADMIN_ITEM_SET TAIS  WHERE ROS.STUDENT_ID = {studentId}    AND ROS.ACTIVATION_STATUS = 'AC'    AND ROS.COMPLETION_DATE_TIME =        (SELECT MAX(COMPLETION_DATE_TIME)           FROM TEST_ROSTER TR          WHERE TR.STUDENT_ID = ROS.STUDENT_ID)    AND ROS.TEST_ADMIN_ID = TA.TEST_ADMIN_ID    AND ROS.TEST_COMPLETION_STATUS IN ('CO', 'IS', 'IC', 'IN')    AND TA.PRODUCT_ID = PROD.PRODUCT_ID    AND STU.STUDENT_ID = ROS.STUDENT_ID    AND ISET.ACTIVATION_STATUS = 'AC'    AND ISET.ITEM_SET_LEVEL <> 'L'    AND ISET.SAMPLE <> 'T'    AND ISET.ITEM_SET_ID = ISP.ITEM_SET_ID    AND ISP.PARENT_ITEM_SET_ID = TA.ITEM_SET_ID    AND TAIS.ITEM_SET_ID = ISET.ITEM_SET_ID    AND TAIS.TEST_ADMIN_ID = TA.TEST_ADMIN_ID  ORDER BY TAIS.ITEM_SET_ORDER",
             arrayMaxLength = 100000)
