...
{ 
    ...
    */ 
    ...
    // Change For Defect:66007
     ...
     @JdbcControl.SQL(statement = "SELECT STU.LAST_NAME || ', ' || STU.FIRST_NAME AS studentName,            ROS.COMPLETION_DATE_TIME AS completionDateTime,            ROS.TEST_COMPLETION_STATUS AS completionStatus,            ROS.TEST_ROSTER_ID AS testRosterId,            TA.TEST_ADMIN_NAME AS testSessionName,            PROD.PRODUCT_ID AS productId,            PROD.PRODUCT_DESCRIPTION AS productName,            ISET.ITEM_SET_ID AS itemSetId,            ISET.ITEM_SET_TYPE AS itemSetType,            ISET.ITEM_SET_LEVEL AS itemSetLevel,            ISET.ITEM_SET_DISPLAY_NAME AS itemSetName,            DECODE(PROD.PRODUCT_ID, 4010, 4012, 4010) AS recommendedProductId,            (SELECT PRODUCT_NAME               FROM PRODUCT              WHERE PRODUCT_ID = DECODE(PROD.PRODUCT_ID, 4010, 4012, 4010)) AS recommendedProductName       FROM TEST_ROSTER         ROS,            PRODUCT             PROD,            STUDENT             STU,            ITEM_SET            ISET,            ITEM_SET_PARENT     ISP,            TEST_ADMIN          TA,            TEST_ADMIN_ITEM_SET TAIS      WHERE ROS.STUDENT_ID = {studentId}        AND ROS.ACTIVATION_STATUS = 'AC'        AND ROS.COMPLETION_DATE_TIME =            (SELECT MAX(COMPLETION_DATE_TIME)               FROM TEST_ROSTER TR              WHERE TR.STUDENT_ID = ROS.STUDENT_ID)        AND ROS.TEST_ADMIN_ID = TA.TEST_ADMIN_ID        AND ROS.TEST_COMPLETION_STATUS IN ('CO', 'IS', 'IC', 'IN')        AND TA.PRODUCT_ID = PROD.PRODUCT_ID        AND STU.STUDENT_ID = ROS.STUDENT_ID        AND ISET.ACTIVATION_STATUS = 'AC'        AND ISET.ITEM_SET_ID = ISP.ITEM_SET_ID        AND ISP.PARENT_ITEM_SET_ID = TA.ITEM_SET_ID        AND TAIS.ITEM_SET_ID = ISET.ITEM_SET_ID        AND TAIS.TEST_ADMIN_ID = TA.TEST_ADMIN_ID      ORDER BY TAIS.ITEM_SET_ORDER",
             arrayMaxLength = 100000)
