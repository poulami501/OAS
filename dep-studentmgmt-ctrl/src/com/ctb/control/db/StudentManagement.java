...
{ 
   ...
   //Added for TABE BAUM - 028
	...
	@JdbcControl.SQL(statement = "select distinct stu.student_id as id,  stu.user_name as loginId,  stu.first_name as firstName,  stu.middle_name as middleName,  stu.last_name as lastName,  concat(concat(stu.last_name, ', '),         concat(stu.first_name, concat(' ', stu.MIDDLE_NAME))) as studentName,  stu.gender as gender,  stu.birthdate as birthDate,  stu.grade as grade,  stu.ext_pin1 as studentIdNumber,  stu.ext_pin2 as studentIdNumber2,  stu.created_by as createdBy,  RTRIM(xmlagg(xmlelement(d, NODE.ORG_NODE_NAME || ','))        .extract('//text()'),        ',') as orgNodeName,  sac.screen_magnifier as screenMagnifier,  sac.screen_reader as screenReader,  sac.calculator as calculator,  sac.test_pause as testPause,  sac.untimed_test as untimedTest,  sac.question_background_color as questionBackgroundColor,  sac.question_font_color as questionFontColor,  sac.question_font_size as questionFontSize,  sac.answer_background_color as answerBackgroundColor,  sac.answer_font_color as answerFontColor,  sac.answer_font_size as answerFontSize,  sac.highlighter as highlighter,  sac.music_file_id as musicFile,  sac.masking_ruler as maskingRuler,  sac.magnifying_glass as magnifyingGlass,  sac.extended_time as extendedTime,  sac.masking_tool as maskingTool,  sac.microphone_headphone as microphoneHeadphone    from org_node_student      ons,         student               stu,         org_node              node,         org_node_category     onc,         student_accommodation sac   where ons.student_id = stu.student_id     and ons.activation_status = 'AC'     and stu.activation_status = 'AC'     and ons.org_node_id = node.org_node_id     and sac.student_id(+) = ons.student_id     AND ONS.ORG_NODE_ID IN         (SELECT ORG_NODE_ID            FROM ORG_NODE_ANCESTOR ONS           WHERE ANCESTOR_ORG_NODE_ID = {orgNodeId}             AND NUMBER_OF_LEVELS IN (0, 1))     and onc.org_node_category_id = node.org_node_category_id         group by           stu.student_id,  stu.user_name,  stu.first_name,  stu.middle_name,  stu.last_name,  concat(concat(stu.last_name, ', '),         concat(stu.first_name, concat(' ', stu.MIDDLE_NAME))),  stu.gender,  stu.birthdate,  stu.grade,  stu.ext_pin1,  stu.ext_pin2,  stu.created_by,  sac.screen_magnifier,  sac.screen_reader,  sac.calculator,  sac.test_pause,  sac.untimed_test,  sac.question_background_color,  sac.question_font_color,  sac.question_font_size,  sac.answer_background_color,  sac.answer_font_color,  sac.answer_font_size,  sac.highlighter,  sac.music_file_id,  sac.masking_ruler,  sac.magnifying_glass,  sac.extended_time,  sac.masking_tool,  sac.microphone_headphone",
	...
	//@JdbcControl.SQL(statement = "  SELECT DISTINCT STD.STUDENT_ID AS ID,  STD.USER_NAME AS LOGINID, STD.FIRST_NAME AS FIRSTNAME, STD.MIDDLE_NAME AS MIDDLENAME, STD.LAST_NAME AS LASTNAME,                                                                                                             STD.GENDER AS GENDER,                             STD.GRADE AS GRADE, STD.EXT_PIN1 AS STUDENTIDNUMBER, STD.EXT_PIN2 AS STUDENTIDNUMBER2, ACCOM.SCREEN_MAGNIFIER AS SCREENMAGNIFIER, ACCOM.SCREEN_READER AS SCREENREADER,  ACCOM.CALCULATOR AS CALCULATOR,                 ACCOM.TEST_PAUSE AS TESTPAUSE,  ACCOM.UNTIMED_TEST AS UNTIMEDTEST,  ACCOM.QUESTION_BACKGROUND_COLOR AS QUESTIONBACKGROUNDCOLOR, ACCOM.QUESTION_FONT_COLOR AS QUESTIONFONTCOLOR, ACCOM.QUESTION_FONT_SIZE AS QUESTIONFONTSIZE, ACCOM.ANSWER_BACKGROUND_COLOR AS ANSWERBACKGROUNDCOLOR, ACCOM.ANSWER_FONT_COLOR AS ANSWERFONTCOLOR,  ACCOM.ANSWER_FONT_SIZE AS ANSWERFONTSIZE, ACCOM.EXTENDED_TIME AS extendedTime, ACCOM.HIGHLIGHTER AS highlighter,  ACCOM.MASKING_RULER AS maskingRuler,        ACCOM.MAGNIFYING_GLASS AS magnifyingGlass,       ACCOM.MASKING_TOOL AS maskingTool, ACCOM.MICROPHONE_HEADPHONE AS microphoneHeadphone, ACCOM.MUSIC_FILE_ID AS musicFile,                                                                                                       RTRIM   ( xmlagg (xmlelement (d, NODE.ORG_NODE_NAME || ',') ).extract ('//text()') , ',' ) AS orgNameList  FROM ORG_NODE_ANCESTOR     ONA,        ORG_NODE_STUDENT      ONS,        STUDENT               STD,        STUDENT_ACCOMMODATION ACCOM,        ORG_NODE              NODE  WHERE ACCOM.STUDENT_ID(+) = STD.STUDENT_ID    AND ONA.ANCESTOR_ORG_NODE_ID = {orgNodeId}    AND ONS.ORG_NODE_ID = ONA.ORG_NODE_ID    AND STD.STUDENT_ID = ONS.STUDENT_ID    AND NODE.ORG_NODE_ID = ONA.ORG_NODE_ID    AND STD.ACTIVATION_STATUS = 'AC'    AND ONS.ACTIVATION_STATUS = 'AC'    AND NODE.ACTIVATION_STATUS = 'AC' GROUP BY     STD.STUDENT_ID,     STD.USER_NAME ,     STD.FIRST_NAME ,      STD.MIDDLE_NAME,      STD.LAST_NAME,                                                                                                            STD.GENDER,                        STD.GRADE ,     STD.EXT_PIN1 ,     STD.EXT_PIN2 ,   ACCOM.SCREEN_MAGNIFIER ,     ACCOM.SCREEN_READER ,     ACCOM.CALCULATOR ,     ACCOM.TEST_PAUSE ,     ACCOM.UNTIMED_TEST ,     ACCOM.QUESTION_BACKGROUND_COLOR ,     ACCOM.QUESTION_FONT_COLOR ,     ACCOM.QUESTION_FONT_SIZE ,     ACCOM.ANSWER_BACKGROUND_COLOR ,     ACCOM.ANSWER_FONT_COLOR ,     ACCOM.ANSWER_FONT_SIZE ,     ACCOM.EXTENDED_TIME ,     ACCOM.HIGHLIGHTER ,     ACCOM.MASKING_RULER ,     ACCOM.MAGNIFYING_GLASS ,     ACCOM.MASKING_TOOL , ACCOM.MICROPHONE_HEADPHONE, ACCOM.MUSIC_FILE_ID ORDER BY STD.LAST_NAME",
	@JdbcControl.SQL(statement = "  SELECT DISTINCT STD.STUDENT_ID AS ID,  STD.USER_NAME AS LOGINID, STD.FIRST_NAME AS FIRSTNAME, STD.MIDDLE_NAME AS MIDDLENAME, STD.LAST_NAME AS LASTNAME, NVL(STD.OUT_OF_SCHOOL, 'No') AS outOfSchool,                                                                                                           STD.GENDER AS GENDER,                             STD.GRADE AS GRADE, STD.EXT_PIN1 AS STUDENTIDNUMBER, STD.EXT_PIN2 AS STUDENTIDNUMBER2, ACCOM.SCREEN_MAGNIFIER AS SCREENMAGNIFIER, ACCOM.SCREEN_READER AS SCREENREADER,  ACCOM.CALCULATOR AS CALCULATOR,                 ACCOM.TEST_PAUSE AS TESTPAUSE,  ACCOM.UNTIMED_TEST AS UNTIMEDTEST,  ACCOM.QUESTION_BACKGROUND_COLOR AS QUESTIONBACKGROUNDCOLOR, ACCOM.QUESTION_FONT_COLOR AS QUESTIONFONTCOLOR, ACCOM.QUESTION_FONT_SIZE AS QUESTIONFONTSIZE, ACCOM.ANSWER_BACKGROUND_COLOR AS ANSWERBACKGROUNDCOLOR, ACCOM.ANSWER_FONT_COLOR AS ANSWERFONTCOLOR,  ACCOM.ANSWER_FONT_SIZE AS ANSWERFONTSIZE, ACCOM.EXTENDED_TIME AS extendedTime, ACCOM.HIGHLIGHTER AS highlighter,  ACCOM.MASKING_RULER AS maskingRuler,        ACCOM.MAGNIFYING_GLASS AS magnifyingGlass,       ACCOM.MASKING_TOOL AS maskingTool, ACCOM.MICROPHONE_HEADPHONE AS microphoneHeadphone, ACCOM.MUSIC_FILE_ID AS musicFile,                                                                                                       RTRIM   ( xmlagg (xmlelement (d, NODE.ORG_NODE_NAME || ',') ).extract ('//text()') , ',' ) AS orgNameList  FROM ORG_NODE_ANCESTOR     ONA,        ORG_NODE_STUDENT      ONS,        STUDENT               STD,        STUDENT_ACCOMMODATION ACCOM,        ORG_NODE              NODE  WHERE ACCOM.STUDENT_ID(+) = STD.STUDENT_ID    AND ONA.ANCESTOR_ORG_NODE_ID = {orgNodeId}    AND ONS.ORG_NODE_ID = ONA.ORG_NODE_ID    AND STD.STUDENT_ID = ONS.STUDENT_ID    AND NODE.ORG_NODE_ID = ONA.ORG_NODE_ID    AND STD.ACTIVATION_STATUS = 'AC'    AND ONS.ACTIVATION_STATUS = 'AC'    AND NODE.ACTIVATION_STATUS = 'AC' GROUP BY     STD.STUDENT_ID,     STD.USER_NAME ,     STD.FIRST_NAME ,      STD.MIDDLE_NAME,      STD.LAST_NAME,                                                                                                            STD.GENDER,                        STD.GRADE ,     STD.EXT_PIN1 ,     STD.EXT_PIN2 , STD.OUT_OF_SCHOOL,  ACCOM.SCREEN_MAGNIFIER ,     ACCOM.SCREEN_READER ,     ACCOM.CALCULATOR ,     ACCOM.TEST_PAUSE ,     ACCOM.UNTIMED_TEST ,     ACCOM.QUESTION_BACKGROUND_COLOR ,     ACCOM.QUESTION_FONT_COLOR ,     ACCOM.QUESTION_FONT_SIZE ,     ACCOM.ANSWER_BACKGROUND_COLOR ,     ACCOM.ANSWER_FONT_COLOR ,     ACCOM.ANSWER_FONT_SIZE ,     ACCOM.EXTENDED_TIME ,     ACCOM.HIGHLIGHTER ,     ACCOM.MASKING_RULER ,     ACCOM.MAGNIFYING_GLASS ,     ACCOM.MASKING_TOOL , ACCOM.MICROPHONE_HEADPHONE, ACCOM.MUSIC_FILE_ID ORDER BY STD.LAST_NAME",
            arrayMaxLength = 100000)
    ...
    //@JdbcControl.SQL(statement = "SELECT DISTINCT ROS.TEST_ROSTER_ID AS ROSTERID, STD.STUDENT_ID AS ID, STD.USER_NAME AS LOGINID, CONCAT(CONCAT(STD.LAST_NAME, ', '), CONCAT(STD.FIRST_NAME, CONCAT(' ', STD.MIDDLE_NAME))) AS STUDENTNAME, STD.GENDER AS GENDER, STD.GRADE AS GRADE, STD.EXT_PIN1 AS STUDENTIDNUMBER, ADM.TEST_ADMIN_ID AS TESTADMINID,ADM.TEST_ADMIN_NAME AS TESTSESSIONNAME, TC.TEST_NAME AS TESTCATALOGNAME, TC.ITEM_SET_ID AS ITEMSETIDTC, ADM.ACCESS_CODE AS ACCESSCODE, (select count(1) from item_set_item ist, item item, student_item_set_status siss where ist.item_set_id = siss.item_set_id and item.item_id = ist.item_id and item.item_type = 'CR' and siss.test_roster_id = ROS.test_roster_id and siss.completion_status in ('CO', 'IS', 'IC') and siss.validation_status = 'VA' and siss.exemptions <> 'Y' and siss.absent <> 'Y' and (item.answer_area is null or item.answer_area = 'AudioItem')) as itemCountCRAI, RTRIM(xmlagg(xmlelement(d, NODE.ORG_NODE_NAME || ',')).extract('//text()'), ',') AS orgNameList FROM ORG_NODE_ANCESTOR ONA, ORG_NODE_STUDENT ONS, STUDENT STD, ORG_NODE NODE, TEST_ROSTER ROS, TEST_ADMIN ADM, TEST_CATALOG TC, STUDENT_ITEM_SET_STATUS SISS WHERE ONA.ANCESTOR_ORG_NODE_ID = {orgNodeId} AND ONS.ORG_NODE_ID = ONA.ORG_NODE_ID AND STD.STUDENT_ID = ONS.STUDENT_ID AND NODE.ORG_NODE_ID = ONA.ORG_NODE_ID AND STD.ACTIVATION_STATUS = 'AC' AND ONS.ACTIVATION_STATUS = 'AC' AND NODE.ACTIVATION_STATUS = 'AC' AND ROS.STUDENT_ID = STD.STUDENT_ID AND ROS.ACTIVATION_STATUS = 'AC' AND ROS.VALIDATION_STATUS IN ('VA','PI') AND ROS.TEST_ROSTER_ID = SISS.TEST_ROSTER_ID AND SISS.COMPLETION_STATUS IN ('CO', 'IS', 'IC')  AND NVL(ROS.STUDENT_EXPORTED, 'F') = 'F' AND ADM.TEST_ADMIN_ID = ROS.TEST_ADMIN_ID AND ADM.ITEM_SET_ID = TC.ITEM_SET_ID GROUP BY ROS.TEST_ROSTER_ID, STD.STUDENT_ID, STD.USER_NAME, CONCAT(CONCAT(STD.LAST_NAME, ', '), CONCAT(STD.FIRST_NAME, CONCAT(' ', STD.MIDDLE_NAME))), STD.GENDER, STD.GRADE, STD.EXT_PIN1, ADM.TEST_ADMIN_ID, ADM.TEST_ADMIN_NAME, TC.TEST_NAME, TC.ITEM_SET_ID, ADM.ACCESS_CODE ORDER BY INITCAP(NVL(STD.EXT_PIN1,0))",
    @JdbcControl.SQL(statement = "SELECT ROSTERID, ID, LOGINID, STUDENTNAME, GENDER, GRADE, STUDENTIDNUMBER, TESTADMINID, TESTSESSIONNAME, TESTCATALOGNAME, ITEMSETIDTC, ACCESSCODE, ITEMCOUNTCRAI, RTRIM(XMLAGG(XMLELEMENT(D, ORG_NODE_NAME || ',')).EXTRACT('//text()'), ',') AS ORGNAMELIST, decode(OAS_UTILS.GET_SCORING_STATUS_BY_ROSTER(ROSTERID),'IN','Incomplete','Complete') SCORINGSTATUS FROM (SELECT DISTINCT ROS.TEST_ROSTER_ID AS ROSTERID, STD.STUDENT_ID AS ID, STD.USER_NAME AS LOGINID, CONCAT(CONCAT(STD.LAST_NAME, ', '), CONCAT(STD.FIRST_NAME, CONCAT(' ', STD.MIDDLE_NAME))) AS STUDENTNAME, STD.GENDER AS GENDER, STD.GRADE AS GRADE, STD.EXT_PIN1 AS STUDENTIDNUMBER, ADM.TEST_ADMIN_ID AS TESTADMINID, ADM.TEST_ADMIN_NAME AS TESTSESSIONNAME, TC.TEST_NAME AS TESTCATALOGNAME, TC.ITEM_SET_ID AS ITEMSETIDTC, ADM.ACCESS_CODE AS ACCESSCODE, (select count(1) from item_set_item    ist, item      item, student_item_set_status siss where ist.item_set_id = siss.item_set_id and item.item_id = ist.item_id and item.item_type = 'CR' and siss.test_roster_id = ROS.test_roster_id and siss.completion_status in ('CO', 'IS', 'IC') and siss.validation_status = 'VA' and siss.exemptions <> 'Y' and siss.absent <> 'Y' and (item.answer_area is null OR item.answer_area = 'AudioItem')) as itemCountCRAI, NODE.ORG_NODE_NAME AS ORG_NODE_NAME FROM ORG_NODE_ANCESTOR ONA, ORG_NODE_STUDENT ONS, STUDENT   STD, ORG_NODE  NODE, TEST_ROSTER      ROS, TEST_ADMIN ADM, TEST_CATALOG     TC, USERS     USR, USER_ROLE UROLE, ORG_NODE_ANCESTOR ONAB, TEST_ADMIN_USER_ROLE    TAUR, STUDENT_ITEM_SET_STATUS SISS WHERE ONA.ANCESTOR_ORG_NODE_ID = {orgNodeId} AND ONS.ORG_NODE_ID = ONA.ORG_NODE_ID AND STD.STUDENT_ID = ONS.STUDENT_ID AND NODE.ORG_NODE_ID = ONA.ORG_NODE_ID AND STD.ACTIVATION_STATUS = 'AC' AND ONS.ACTIVATION_STATUS = 'AC' AND NODE.ACTIVATION_STATUS = 'AC' AND ROS.STUDENT_ID = STD.STUDENT_ID AND ROS.ACTIVATION_STATUS = 'AC' AND USR.USER_NAME = {userName} AND USR.USER_ID = UROLE.USER_ID AND UROLE.ORG_NODE_ID = ONAB.ANCESTOR_ORG_NODE_ID AND TAUR.TEST_ADMIN_ID = ADM.TEST_ADMIN_ID AND (ONAB.ORG_NODE_ID = ADM.CREATOR_ORG_NODE_ID OR TAUR.USER_ID = USR.USER_ID) AND ROS.VALIDATION_STATUS IN ('VA', 'PI') AND ROS.TEST_ROSTER_ID = SISS.TEST_ROSTER_ID AND SISS.COMPLETION_STATUS IN ('CO', 'IS', 'IC') AND (SISS.EXEMPTIONS <> 'Y' AND SISS.ABSENT <> 'Y') AND NVL(ROS.STUDENT_EXPORTED, 'F') = 'F' AND ADM.TEST_ADMIN_ID = ROS.TEST_ADMIN_ID AND ADM.ITEM_SET_ID = TC.ITEM_SET_ID) DERIVED GROUP BY ROSTERID, ID, LOGINID, STUDENTNAME, GENDER, GRADE, STUDENTIDNUMBER, TESTADMINID, TESTSESSIONNAME, TESTCATALOGNAME, ITEMSETIDTC, ACCESSCODE, ITEMCOUNTCRAI ORDER BY INITCAP(NVL(LOGINID, 0))",
            arrayMaxLength = 0, fetchSize = 100)
     ManageStudent [] getStudentsMinimalInfoForStudentScoring(Integer orgNodeId, String userName) throws SQLException;

    //@JdbcControl.SQL(statement = "SELECT ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTCATALOGNAME , STUDENTIDNUMBER, ORGNAMELIST, CONTENTAREASTRING FROM ( SELECT ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTNAME AS TESTCATALOGNAME, STUDENTIDNUMBER, RTRIM(XMLAGG(XMLELEMENT(D, ORG || ',')).EXTRACT('//text()'), ',') AS ORGNAMELIST, OAS_UTILS.GET_STDS_SCORING_STATUS(ROSTERID) AS CONTENTAREASTRING FROM ( SELECT DISTINCT ROS.TEST_ROSTER_ID AS ROSTERID, 	       STD.STUDENT_ID AS ID, 	       STD.USER_NAME AS LOGINID, 	       STD.FIRST_NAME AS FIRSTNAME, 	       STD.MIDDLE_NAME AS MIDDLENAME, 	       STD.LAST_NAME AS LASTNAME, 	       CONCAT(CONCAT(STD.FIRST_NAME, ' '), STD.LAST_NAME) AS STUDENTNAME, 	       STD.GRADE AS GRADE, 	       ADM.TEST_ADMIN_NAME AS TESTSESSIONNAME, 	       ADM.TEST_ADMIN_ID AS TESTADMINID, 	       NODE.ORG_NODE_NAME AS ORG, 	       TC.TEST_NAME       AS TESTNAME  ,  STD.EXT_PIN1 AS STUDENTIDNUMBER FROM ORG_NODE_ANCESTOR       ONA,       ORG_NODE_STUDENT        ONS,       STUDENT                 STD,       ORG_NODE                NODE,       TEST_ROSTER             ROS,       TEST_ADMIN              ADM,       TEST_CATALOG            TC,       STUDENT_ITEM_SET_STATUS SISS,       USERS                   USR,       USER_ROLE               UROLE,       ORG_NODE_ANCESTOR       ONAB, TEST_ADMIN_USER_ROLE TAUR  WHERE ONA.ANCESTOR_ORG_NODE_ID = {orgNodeId}   AND ONS.ORG_NODE_ID = ONA.ORG_NODE_ID   AND STD.STUDENT_ID = ONS.STUDENT_ID   AND NODE.ORG_NODE_ID = ONA.ORG_NODE_ID   AND ROS.STUDENT_ID = STD.STUDENT_ID   AND ROS.TEST_ROSTER_ID = SISS.TEST_ROSTER_ID   AND ADM.TEST_ADMIN_ID = ROS.TEST_ADMIN_ID   AND ADM.ITEM_SET_ID = TC.ITEM_SET_ID  AND USR.USER_NAME = {userName}   AND USR.USER_ID = UROLE.USER_ID   AND UROLE.ORG_NODE_ID = ONAB.ANCESTOR_ORG_NODE_ID    AND TAUR.TEST_ADMIN_ID = ADM.TEST_ADMIN_ID AND ( ONAB.ORG_NODE_ID = ADM.CREATOR_ORG_NODE_ID OR TAUR.USER_ID= USR.USER_ID)  AND STD.ACTIVATION_STATUS = 'AC'   AND ONS.ACTIVATION_STATUS = 'AC'   AND NODE.ACTIVATION_STATUS = 'AC'   AND ROS.VALIDATION_STATUS IN ('VA', 'PI')   AND SISS.COMPLETION_STATUS IN ('CO', 'IS', 'IC')   AND (SISS.EXEMPTIONS <> 'Y' AND SISS.ABSENT <> 'Y') ) DERIVED1 GROUP BY ROSTERID,  ID,   LOGINID,  FIRSTNAME, MIDDLENAME,   LASTNAME,  STUDENTNAME,  GRADE, TESTSESSIONNAME, TESTADMINID, TESTNAME, STUDENTIDNUMBER ORDER BY LOGINID ) DERIVED2 WHERE CONTENTAREASTRING IS NOT NULL",
    //@JdbcControl.SQL(statement = "SELECT ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTCATALOGNAME, STUDENTIDNUMBER, ORGNAMELIST, CONTENTAREASTRING, FORM, LOGIN_START_DATE AS ADMINISTRATIONDATE, CREATEDBY FROM (SELECT ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTNAME AS TESTCATALOGNAME, STUDENTIDNUMBER, RTRIM(XMLAGG(XMLELEMENT(D, ORG || ',')).EXTRACT('//text()'), ',') AS ORGNAMELIST, OAS_UTILS.GET_STDS_SCORING_STATUS(ROSTERID) AS CONTENTAREASTRING, FORM, LOGIN_START_DATE, CREATEDBY FROM (SELECT DISTINCT ROS.TEST_ROSTER_ID AS ROSTERID, STD.STUDENT_ID AS ID, STD.USER_NAME AS LOGINID, STD.FIRST_NAME AS FIRSTNAME, STD.MIDDLE_NAME AS MIDDLENAME, STD.LAST_NAME AS LASTNAME, CONCAT(CONCAT(STD.FIRST_NAME, ' '), STD.LAST_NAME) AS STUDENTNAME, STD.GRADE AS GRADE, ADM.TEST_ADMIN_NAME AS TESTSESSIONNAME, ADM.TEST_ADMIN_ID AS TESTADMINID, NODE.ORG_NODE_NAME AS ORG, TC.TEST_NAME AS TESTNAME, STD.EXT_PIN1 AS STUDENTIDNUMBER, ROS.FORM_ASSIGNMENT AS FORM, ADM.LOGIN_START_DATE AS LOGIN_START_DATE, ROS.CREATED_BY AS CREATEDBY FROM ORG_NODE_ANCESTOR ONA, ORG_NODE_STUDENT ONS, STUDENT STD, ORG_NODE NODE, TEST_ROSTER ROS, TEST_ADMIN ADM, TEST_CATALOG TC, STUDENT_ITEM_SET_STATUS SISS WHERE ONA.ANCESTOR_ORG_NODE_ID = {orgNodeId} AND ONS.ORG_NODE_ID = ONA.ORG_NODE_ID AND STD.STUDENT_ID = ONS.STUDENT_ID AND NODE.ORG_NODE_ID = ONA.ORG_NODE_ID AND ROS.STUDENT_ID = STD.STUDENT_ID AND ROS.TEST_ROSTER_ID = SISS.TEST_ROSTER_ID AND ADM.TEST_ADMIN_ID = ROS.TEST_ADMIN_ID AND ADM.ITEM_SET_ID = TC.ITEM_SET_ID AND STD.ACTIVATION_STATUS = 'AC' AND ONS.ACTIVATION_STATUS = 'AC' AND NODE.ACTIVATION_STATUS = 'AC' AND ROS.VALIDATION_STATUS IN ('VA', 'PI') AND SISS.COMPLETION_STATUS IN ('CO', 'IS', 'IC') AND (SISS.EXEMPTIONS <> 'Y' AND SISS.ABSENT <> 'Y')) DERIVED1 GROUP BY ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTNAME, STUDENTIDNUMBER, FORM, LOGIN_START_DATE, CREATEDBY ORDER BY LOGINID) DERIVED2 WHERE CONTENTAREASTRING IS NOT NULL",
    //@JdbcControl.SQL(statement = "SELECT ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTCATALOGNAME, STUDENTIDNUMBER, ORGNAMELIST, CONTENTAREASTRING, FORM, LOGIN_START_DATE AS ADMINISTRATIONDATE, DEFAULTSCHEDULER FROM (SELECT ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTNAME AS TESTCATALOGNAME, STUDENTIDNUMBER, RTRIM(XMLAGG(XMLELEMENT(D, ORG || ',')).EXTRACT('//text()'), ',') AS ORGNAMELIST, OAS_UTILS.GET_STDS_SCORING_STATUS(ROSTERID) AS CONTENTAREASTRING, FORM, LOGIN_START_DATE, DEFAULTSCHEDULER FROM (SELECT DISTINCT ROS.TEST_ROSTER_ID AS ROSTERID, STD.STUDENT_ID AS ID, STD.USER_NAME AS LOGINID, STD.FIRST_NAME AS FIRSTNAME, STD.MIDDLE_NAME AS MIDDLENAME, STD.LAST_NAME AS LASTNAME, CONCAT(CONCAT(STD.FIRST_NAME, ' '), STD.LAST_NAME) AS STUDENTNAME, STD.GRADE AS GRADE, ADM.TEST_ADMIN_NAME AS TESTSESSIONNAME, ADM.TEST_ADMIN_ID AS TESTADMINID, NODE.ORG_NODE_NAME AS ORG, TC.TEST_NAME AS TESTNAME, STD.EXT_PIN1 AS STUDENTIDNUMBER, ROS.FORM_ASSIGNMENT AS FORM, ADM.LOGIN_START_DATE AS LOGIN_START_DATE, USR.USER_NAME AS DEFAULTSCHEDULER FROM ORG_NODE_ANCESTOR ONA, ORG_NODE_STUDENT ONS, STUDENT STD, ORG_NODE NODE, TEST_ROSTER ROS, TEST_ADMIN ADM, TEST_CATALOG TC, STUDENT_ITEM_SET_STATUS SISS, USERS USR WHERE ONA.ANCESTOR_ORG_NODE_ID = {orgNodeId} AND ONS.ORG_NODE_ID = ONA.ORG_NODE_ID AND STD.STUDENT_ID = ONS.STUDENT_ID AND NODE.ORG_NODE_ID = ONA.ORG_NODE_ID AND ROS.STUDENT_ID = STD.STUDENT_ID AND ROS.TEST_ROSTER_ID = SISS.TEST_ROSTER_ID AND ADM.TEST_ADMIN_ID = ROS.TEST_ADMIN_ID AND ADM.ITEM_SET_ID = TC.ITEM_SET_ID AND ADM.CREATED_BY = USR.USER_ID AND STD.ACTIVATION_STATUS = 'AC' AND ONS.ACTIVATION_STATUS = 'AC' AND NODE.ACTIVATION_STATUS = 'AC' AND ROS.VALIDATION_STATUS IN ('VA', 'PI') AND SISS.COMPLETION_STATUS IN ('CO', 'IS', 'IC') AND (SISS.EXEMPTIONS <> 'Y' AND SISS.ABSENT <> 'Y')) DERIVED1 GROUP BY ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTNAME, STUDENTIDNUMBER, FORM, LOGIN_START_DATE, DEFAULTSCHEDULER ORDER BY LOGINID) DERIVED2 WHERE CONTENTAREASTRING IS NOT NULL",
    //@JdbcControl.SQL(statement = "SELECT ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTCATALOGNAME, STUDENTIDNUMBER, ORGNAMELIST, CONTENTAREASTRING, FORM, LOGIN_START_DATE AS ADMINISTRATIONDATE, DEFAULTSCHEDULER FROM (SELECT ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTNAME AS TESTCATALOGNAME, STUDENTIDNUMBER, RTRIM(XMLAGG(XMLELEMENT(D, ORG || ',')).EXTRACT('//text()'), ',') AS ORGNAMELIST, OAS_UTILS.GET_STDS_SCORING_STATUS(ROSTERID) AS CONTENTAREASTRING, FORM, LOGIN_START_DATE, DEFAULTSCHEDULER FROM (SELECT DISTINCT ROS.TEST_ROSTER_ID AS ROSTERID, STD.STUDENT_ID AS ID, STD.USER_NAME AS LOGINID, STD.FIRST_NAME AS FIRSTNAME, STD.MIDDLE_NAME AS MIDDLENAME, STD.LAST_NAME AS LASTNAME, CONCAT(CONCAT(STD.FIRST_NAME, ' '), STD.LAST_NAME) AS STUDENTNAME, STD.GRADE AS GRADE, ADM.TEST_ADMIN_NAME AS TESTSESSIONNAME, ADM.TEST_ADMIN_ID AS TESTADMINID, NODE.ORG_NODE_NAME AS ORG, TC.TEST_NAME AS TESTNAME, STD.EXT_PIN1 AS STUDENTIDNUMBER, ROS.FORM_ASSIGNMENT AS FORM, ADM.LOGIN_START_DATE AS LOGIN_START_DATE, USR.USER_NAME AS DEFAULTSCHEDULER FROM ORG_NODE_ANCESTOR ONA, ORG_NODE_STUDENT ONS, STUDENT STD, ORG_NODE NODE, TEST_ROSTER ROS, TEST_ADMIN ADM, TEST_CATALOG TC, STUDENT_ITEM_SET_STATUS SISS, USERS USR, PRODUCT PROD WHERE ONA.ANCESTOR_ORG_NODE_ID = {orgNodeId} AND PROD.PARENT_PRODUCT_ID = {productId} AND PROD.PRODUCT_ID = ADM.PRODUCT_ID AND ONS.ORG_NODE_ID = ONA.ORG_NODE_ID AND STD.STUDENT_ID = ONS.STUDENT_ID AND NODE.ORG_NODE_ID = ONA.ORG_NODE_ID AND ROS.STUDENT_ID = STD.STUDENT_ID AND ROS.TEST_ROSTER_ID = SISS.TEST_ROSTER_ID AND ADM.TEST_ADMIN_ID = ROS.TEST_ADMIN_ID AND ADM.ITEM_SET_ID = TC.ITEM_SET_ID AND ADM.CREATED_BY = USR.USER_ID AND STD.ACTIVATION_STATUS = 'AC' AND ONS.ACTIVATION_STATUS = 'AC' AND NODE.ACTIVATION_STATUS = 'AC' AND ROS.VALIDATION_STATUS IN ('VA', 'PI') AND SISS.COMPLETION_STATUS IN ('CO', 'IS', 'IC') AND (SISS.EXEMPTIONS <> 'Y' AND SISS.ABSENT <> 'Y')) DERIVED1 GROUP BY ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTNAME, STUDENTIDNUMBER, FORM, LOGIN_START_DATE, DEFAULTSCHEDULER ORDER BY LOGINID) DERIVED2 WHERE CONTENTAREASTRING IS NOT NULL",
    @JdbcControl.SQL(statement = "SELECT ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTCATALOGNAME, STUDENTIDNUMBER, ORGNAMELIST, OAS_UTILS.FILTER_COMPLETED_SUBTEST_STR(ROSTERID, CONTENTAREASTRING) AS CONTENTAREASTRING, FORM, LOGIN_START_DATE AS ADMINISTRATIONDATE, DEFAULTSCHEDULER FROM (SELECT ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTNAME AS TESTCATALOGNAME, STUDENTIDNUMBER, RTRIM(XMLAGG(XMLELEMENT(D, ORG || ',')).EXTRACT('//text()'), ',') AS ORGNAMELIST, OAS_UTILS.GET_STDS_SCORING_STATUS(ROSTERID) AS CONTENTAREASTRING, FORM, LOGIN_START_DATE, DEFAULTSCHEDULER FROM (SELECT DISTINCT ROS.TEST_ROSTER_ID AS ROSTERID, STD.STUDENT_ID AS ID, STD.USER_NAME AS LOGINID, STD.FIRST_NAME AS FIRSTNAME, STD.MIDDLE_NAME AS MIDDLENAME, STD.LAST_NAME AS LASTNAME, CONCAT(CONCAT(STD.FIRST_NAME, ' '), STD.LAST_NAME) AS STUDENTNAME, STD.GRADE AS GRADE, ADM.TEST_ADMIN_NAME AS TESTSESSIONNAME, ADM.TEST_ADMIN_ID AS TESTADMINID, NODE.ORG_NODE_NAME AS ORG, TC.TEST_NAME AS TESTNAME, STD.EXT_PIN1 AS STUDENTIDNUMBER, ROS.FORM_ASSIGNMENT AS FORM, ROS.START_DATE_TIME AS LOGIN_START_DATE, USR.USER_NAME AS DEFAULTSCHEDULER FROM ORG_NODE_ANCESTOR       ONA, ORG_NODE_STUDENT        ONS, STUDENT                 STD, ORG_NODE                NODE, TEST_ROSTER             ROS, TEST_ADMIN              ADM, TEST_CATALOG            TC, STUDENT_ITEM_SET_STATUS SISS, USERS                   USR, PRODUCT                 PROD WHERE ONA.ANCESTOR_ORG_NODE_ID = {orgNodeId} AND PROD.PARENT_PRODUCT_ID = {productId} AND PROD.PRODUCT_ID = ADM.PRODUCT_ID AND ONS.ORG_NODE_ID = ONA.ORG_NODE_ID AND STD.STUDENT_ID = ONS.STUDENT_ID AND NODE.ORG_NODE_ID = ONA.ORG_NODE_ID AND ROS.STUDENT_ID = STD.STUDENT_ID AND ROS.TEST_ROSTER_ID = SISS.TEST_ROSTER_ID AND ADM.TEST_ADMIN_ID = ROS.TEST_ADMIN_ID AND ADM.ITEM_SET_ID = TC.ITEM_SET_ID AND ADM.CREATED_BY = USR.USER_ID AND STD.ACTIVATION_STATUS = 'AC' AND ONS.ACTIVATION_STATUS = 'AC' AND NODE.ACTIVATION_STATUS = 'AC' AND ROS.VALIDATION_STATUS IN ('VA', 'PI') AND SISS.COMPLETION_STATUS IN ('CO', 'IS', 'IC') AND (SISS.EXEMPTIONS <> 'Y' AND SISS.ABSENT <> 'Y')) DERIVED1 GROUP BY ROSTERID, ID, LOGINID, FIRSTNAME, MIDDLENAME, LASTNAME, STUDENTNAME, GRADE, TESTSESSIONNAME, TESTADMINID, TESTNAME, STUDENTIDNUMBER, FORM, LOGIN_START_DATE, DEFAULTSCHEDULER ORDER BY LOGINID) DERIVED2 WHERE CONTENTAREASTRING IS NOT NULL",
            arrayMaxLength = 0, fetchSize = 100)
    ManageStudent[] getAllCompletedStudentForOrgNode(Integer orgNodeId, Integer productId) throws SQLException;
   
    @JdbcControl.SQL(statement = "SELECT COUNT(DISTINCT ROS.TEST_ROSTER_ID)  FROM ORG_NODE_ANCESTOR       ONA, ORG_NODE_STUDENT        ONS, STUDENT                   STD, ORG_NODE                NODE, TEST_ROSTER             ROS, TEST_ADMIN                ADM, TEST_CATALOG            TC, STUDENT_ITEM_SET_STATUS SISS, USERS                     USR, USER_ROLE               UROLE, ORG_NODE_ANCESTOR       ONAB, TEST_ADMIN_USER_ROLE    TAUR WHERE   ONA.ANCESTOR_ORG_NODE_ID = {orgNodeId} AND ONS.ORG_NODE_ID = ONA.ORG_NODE_ID   AND STD.STUDENT_ID = ONS.STUDENT_ID AND NODE.ORG_NODE_ID = ONA.ORG_NODE_ID AND   ROS.STUDENT_ID = STD.STUDENT_ID AND ROS.TEST_ROSTER_ID = SISS.TEST_ROSTER_ID   AND ADM.TEST_ADMIN_ID = ROS.TEST_ADMIN_ID AND ADM.ITEM_SET_ID = TC.ITEM_SET_ID   AND USR.USER_NAME = {userName} AND USR.USER_ID = UROLE.USER_ID AND   UROLE.ORG_NODE_ID = ONAB.ANCESTOR_ORG_NODE_ID AND TAUR.TEST_ADMIN_ID = ADM.TEST_ADMIN_ID AND ( ONAB.ORG_NODE_ID = ADM.CREATOR_ORG_NODE_ID OR TAUR.USER_ID = USR.USER_ID) AND STD.ACTIVATION_STATUS = 'AC' AND   ONS.ACTIVATION_STATUS = 'AC' AND NODE.ACTIVATION_STATUS = 'AC' AND ROS.VALIDATION_STATUS IN ('VA', 'PI') AND   SISS.COMPLETION_STATUS IN ('CO', 'IS', 'IC') AND (SISS.EXEMPTIONS <> 'Y' AND   SISS.ABSENT <> 'Y') AND OAS_UTILS.GET_STDS_SCORING_STATUS(ROS.TEST_ROSTER_ID) IS NOT NULL ",
            arrayMaxLength = 1)
	Integer getCompletedStudentCountForOrgNode(String userName,	Integer orgNodeId) throws SQLException;
    
    @JdbcControl.SQL(statement = "SELECT STU.FIRST_NAME AS STUDENTFIRSTNAME, STU.LAST_NAME AS STUDENTLASTNAME, CONCAT(CONCAT(STU.FIRST_NAME, ' '), STU.LAST_NAME) AS STUDENTNAME, STU.STUDENT_ID AS STUDENTID, STU.EXT_PIN1 AS STUDENTEXTPIN1, ROS.START_DATE_TIME AS TESTADMINSTARTDATE, ADM.TEST_ADMIN_NAME AS TESTADMINNAME, ROS.FORM_ASSIGNMENT AS FORM, STU.GRADE AS GRADE, ORGSCH.ORG_NODE_NAME AS SCHOOL, ORGDIS.ORG_NODE_NAME AS DISTRICT, TCAT.TEST_NAME AS TESTNAME, OAS_UTILS.GET_STDS_SCORING_STATUS({testRosterId}) AS CONTENTAREANAMESTRING FROM STUDENT STU, TEST_ROSTER ROS, ORG_NODE ORG, ORG_NODE ORGSCH, ORG_NODE_ANCESTOR ONA, ORG_NODE_ANCESTOR ONADIST, TEST_ADMIN ADM, ORG_NODE ORGDIS, TEST_CATALOG TCAT WHERE STU.STUDENT_ID = ROS.STUDENT_ID AND ROS.TEST_ROSTER_ID = {testRosterId} AND ROS.TEST_ADMIN_ID = ADM.TEST_ADMIN_ID AND ROS.ORG_NODE_ID = ORG.ORG_NODE_ID AND ORG.ORG_NODE_ID = ONA.ORG_NODE_ID AND ONA.NUMBER_OF_LEVELS = 1 AND ONA.ANCESTOR_ORG_NODE_ID = ORGSCH.ORG_NODE_ID AND ONA.ORG_NODE_ID = ONADIST.ORG_NODE_ID AND ONADIST.NUMBER_OF_LEVELS = 2 AND ONADIST.ANCESTOR_ORG_NODE_ID = ORGDIS.ORG_NODE_ID AND ADM.TEST_CATALOG_ID = TCAT.TEST_CATALOG_ID",
    ...
    @JdbcControl.SQL(statement = " SELECT DER1.ID AS ID, DER1.LOGINID LOGINID, DER1.STUDENTNAME AS STUDENTNAME, DER1.GENDER AS GENDER, DER1.GRADE AS GRADE, DER1.STUDENTIDNUMBER AS STUDENTIDNUMBER, DER1.ORGANIZATIONNAMES AS ORGANIZATIONNAMES, DECODE(DER2.STUDENT_ID, NULL, 'F', 'T') AS ISSESSIONSTUDENT , orgNameList, orgIdList, orgIdNameList  FROM (SELECT DISTINCT STU.STUDENT_ID AS ID, STU.USER_NAME AS LOGINID, CONCAT(CONCAT(STU.LAST_NAME, ', '), CONCAT(STU.FIRST_NAME, CONCAT(' ', STU.MIDDLE_NAME))) AS STUDENTNAME, STU.GENDER AS GENDER, STU.GRADE AS GRADE, STU.EXT_PIN1 AS STUDENTIDNUMBER, (RTRIM(XMLAGG(XMLELEMENT(D, NODE.ORG_NODE_NAME || ', ')).EXTRACT('//text()'), ', ')) ORGANIZATIONNAMES  ,(RTRIM(XMLAGG(XMLELEMENT(D,   NODE.ORG_NODE_NAME || '| ')).EXTRACT('//text()'),'| ')) orgNameList,   (RTRIM(XMLAGG(XMLELEMENT(D,   NODE.ORG_NODE_ID || '|')).EXTRACT('//text()'),'|')) orgIdList, (RTRIM(XMLAGG(XMLELEMENT(D, NODE.ORG_NODE_ID || '$' || NODE.ORG_NODE_NAME || '|')).EXTRACT('//text()'),'|')) orgIdNameList FROM ORG_NODE_STUDENT  ONS, STUDENT           STU, ORG_NODE           NODE, ORG_NODE_ANCESTOR ONA WHERE ONS.STUDENT_ID = STU.STUDENT_ID AND           ONS.ACTIVATION_STATUS = 'AC' AND STU.ACTIVATION_STATUS = 'AC' AND           ONS.ORG_NODE_ID = NODE.ORG_NODE_ID AND ONS.ORG_NODE_ID =           ONA.ORG_NODE_ID AND ONA.ANCESTOR_ORG_NODE_ID = {orgNodeId} GROUP BY           STU.STUDENT_ID, STU.USER_NAME, STU.LAST_NAME, STU.FIRST_NAME,           STU.MIDDLE_NAME, STU.GENDER, STU.GRADE, STU.EXT_PIN1 ) DER1, (SELECT           ROS.STUDENT_ID FROM TEST_ROSTER ROS, TEST_ADMIN TAD WHERE           TAD.TEST_ADMIN_ID = {testAdminId} AND ROS.TEST_ADMIN_ID =           TAD.TEST_ADMIN_ID) DER2 WHERE DER1.ID = DER2.STUDENT_ID(+) ORDER BY           DER1.LOGINID",
            arrayMaxLength = 500000)
	ManageStudent[] getStudentsForSelectedOrgNode(Integer orgNodeId, Integer testAdminId) throws SQLException;

    @JdbcControl.SQL(statement = " SELECT DISTINCT STU.STUDENT_ID AS ID, STU.USER_NAME AS LOGINID, CONCAT(CONCAT(STU.LAST_NAME, ', '), CONCAT(STU.FIRST_NAME, CONCAT(' ', STU.MIDDLE_NAME))) AS STUDENTNAME, STU.GENDER AS GENDER, STU.GRADE AS GRADE, STU.EXT_PIN1 AS STUDENTIDNUMBER, (RTRIM(XMLAGG(XMLELEMENT(D, NODE.ORG_NODE_NAME || ', ')).EXTRACT('//text()'), ', ')) ORGANIZATIONNAMES  ,(RTRIM(XMLAGG(XMLELEMENT(D,   NODE.ORG_NODE_NAME || '|')).EXTRACT('//text()'),'|')) orgNameList,   (RTRIM(XMLAGG(XMLELEMENT(D,   NODE.ORG_NODE_ID || '|')).EXTRACT('//text()'),'|')) orgIdList, (RTRIM(XMLAGG(XMLELEMENT(D, NODE.ORG_NODE_ID || '$' || NODE.ORG_NODE_NAME || '|')).EXTRACT('//text()'),'|')) orgIdNameList FROM ORG_NODE_STUDENT  ONS, STUDENT           STU, ORG_NODE           NODE, ORG_NODE_ANCESTOR ONA WHERE ONS.STUDENT_ID = STU.STUDENT_ID AND           ONS.ACTIVATION_STATUS = 'AC' AND STU.ACTIVATION_STATUS = 'AC' AND           ONS.ORG_NODE_ID = NODE.ORG_NODE_ID AND ONS.ORG_NODE_ID =           ONA.ORG_NODE_ID AND ONA.ANCESTOR_ORG_NODE_ID = {orgNodeId} GROUP BY           STU.STUDENT_ID, STU.USER_NAME, STU.LAST_NAME, STU.FIRST_NAME,           STU.MIDDLE_NAME, STU.GENDER, STU.GRADE, STU.EXT_PIN1  ORDER BY  LOGINID",
    		arrayMaxLength = 0, fetchSize = 100)
	ManageStudent[] getStudentsAtAndBelowForSelectedOrgNode(Integer orgNodeId) throws SQLException;
    
    @JdbcControl.SQL(statement ="SELECT ROS.TEST_ROSTER_ID AS TESTROSTERID, ADM.PRODUCT_ID AS PRODUCTID, STU.FIRST_NAME AS STUDENTFIRSTNAME, STU.LAST_NAME AS STUDENTLASTNAME, CONCAT(CONCAT(STU.FIRST_NAME, ' '), STU.LAST_NAME) AS STUDENTNAME, STU.STUDENT_ID AS STUDENTID, STU.EXT_PIN1 AS STUDENTEXTPIN1, ROS.START_DATE_TIME AS TESTADMINSTARTDATE, ADM.TEST_ADMIN_NAME AS TESTADMINNAME, ROS.FORM_ASSIGNMENT AS FORM, STU.GRADE AS GRADE, ORGSCH.ORG_NODE_NAME AS SCHOOL, ORGDIS.ORG_NODE_NAME AS DISTRICT, TCAT.TEST_NAME AS TESTNAME, OAS_UTILS.GET_STDS_SCORING_STATUS(ROS.TEST_ROSTER_ID) AS CONTENTAREANAMESTRING FROM STUDENT STU, TEST_ROSTER ROS, ORG_NODE ORG, ORG_NODE ORGSCH, ORG_NODE_ANCESTOR ONA, ORG_NODE_ANCESTOR ONADIST, TEST_ADMIN ADM, ORG_NODE ORGDIS, TEST_CATALOG TCAT WHERE STU.STUDENT_ID = ROS.STUDENT_ID AND {sql: testRosterIds} AND ROS.TEST_ADMIN_ID = ADM.TEST_ADMIN_ID AND ROS.ORG_NODE_ID = ORG.ORG_NODE_ID AND ORG.ORG_NODE_ID = ONA.ORG_NODE_ID AND ONA.NUMBER_OF_LEVELS = 1 AND ONA.ANCESTOR_ORG_NODE_ID = ORGSCH.ORG_NODE_ID AND ONA.ORG_NODE_ID = ONADIST.ORG_NODE_ID AND ONADIST.NUMBER_OF_LEVELS = 2 AND ONADIST.ANCESTOR_ORG_NODE_ID = ORGDIS.ORG_NODE_ID AND ADM.TEST_CATALOG_ID = TCAT.TEST_CATALOG_ID ORDER BY STU.FIRST_NAME",
    		arrayMaxLength = 0, fetchSize = 100)
    StudentScoreReport[] getStudentDataForReportByGroup(String testRosterIds)throws SQLException;
    ...
    @JdbcControl.SQL(statement = "select litho_code from test_roster where student_id  = {studentId}  and activation_status = 'AC' and litho_code is not null and rownum = 1")
    ...
    @JdbcControl.SQL(statement = "SELECT STU.FIRST_NAME AS STUDENTFIRSTNAME, STU.LAST_NAME AS STUDENTLASTNAME, CONCAT(CONCAT(STU.FIRST_NAME, ' '), STU.LAST_NAME) AS STUDENTNAME, STU.STUDENT_ID AS STUDENTID, STU.EXT_PIN1 AS STUDENTEXTPIN1, ROS.START_DATE_TIME AS TESTADMINSTARTDATE, ADM.TEST_ADMIN_NAME AS TESTADMINNAME, ROS.FORM_ASSIGNMENT AS FORM, STU.GRADE AS GRADE, ORGSCH.ORG_NODE_NAME AS SCHOOL, ORGDIS.ORG_NODE_NAME AS DISTRICT, TCAT.TEST_NAME AS TESTNAME, OAS_UTILS.GET_STDS_ACADEMIC_SCORE({testRosterId}) AS CONTENTAREANAMESTRING FROM STUDENT STU, TEST_ROSTER ROS, ORG_NODE ORG, ORG_NODE ORGSCH, ORG_NODE_ANCESTOR ONA, ORG_NODE_ANCESTOR ONADIST, TEST_ADMIN ADM, ORG_NODE ORGDIS, TEST_CATALOG TCAT WHERE STU.STUDENT_ID = ROS.STUDENT_ID AND ROS.TEST_ROSTER_ID = {testRosterId} AND ROS.TEST_ADMIN_ID = ADM.TEST_ADMIN_ID AND ROS.ORG_NODE_ID = ORG.ORG_NODE_ID AND ORG.ORG_NODE_ID = ONA.ORG_NODE_ID AND ONA.NUMBER_OF_LEVELS = 1 AND ONA.ANCESTOR_ORG_NODE_ID = ORGSCH.ORG_NODE_ID AND ONA.ORG_NODE_ID = ONADIST.ORG_NODE_ID AND ONADIST.NUMBER_OF_LEVELS = 2 AND ONADIST.ANCESTOR_ORG_NODE_ID = ORGDIS.ORG_NODE_ID AND ADM.TEST_CATALOG_ID = TCAT.TEST_CATALOG_ID",
            arrayMaxLength = 0, fetchSize = 500)
  	StudentScoreReport getStudentDataForAcademicReport(Integer testRosterId) throws SQLException;
    
    /*
      	SELECT TA.TEST_ADMIN_NAME AS testAdminName
		 ...
		 ORDER BY UPPER(TA.TEST_ADMIN_NAME)
     */
    @JdbcControl.SQL(statement = "SELECT TA.TEST_ADMIN_NAME AS testAdminName FROM TEST_ADMIN TA, TEST_ROSTER TR WHERE TA.TEST_ADMIN_ID = TR.TEST_ADMIN_ID AND TA.ACTIVATION_STATUS = 'AC' AND TA.TEST_ADMIN_STATUS = 'PA' AND TR.STUDENT_ID = {studentId} ORDER BY UPPER(TA.TEST_ADMIN_NAME)")
  	String[] getTestSessionsAssignedToStudent(Integer studentId) throws SQLException;
