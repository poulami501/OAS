...
System.out.println("studentIdList.contains(strCell.trim()) : "+studentIdList.contains(strCell.trim()));
		...
		System.out.println("**Inside logical error..New implement--OLD UI**");
			...
			strCell = getCellValue(cell);
				 ...
				 if (cellHeader.getStringCellValue().equalsIgnoreCase(this.subEthnicityLabel)){
					 ...
					 if (isSubEthnicityRequired && strCell.trim().equals("")){
						 ...
						 System.out.println ("**Logical error ethnicity.. do nothing*OLD UI***");
					 }
					 
					 if (!isSubEthnicityRequired && !strCell.trim().equals("")){
						 logicalErrorList.add(CTBConstants.ETHNICITY_LABEL);
						 System.out.println("**Inside logical error..!isSubEthnicityRequired && !strCell.trim().equals('')..*OLD UI***");
					 ...
					 if (!isEthnicityPresent && !strCell.trim().equals("")){
						 ...
						 System.out.println("**Inside logical error..!isEthnicityPresent && !strCell.trim().equals('')*OLD UI***");
	...
	private StudentDemographic [] getStudentDemographicData(
				...
				if(demoCardinality.equals(CTBConstants.MULTIPLE_DEMOGRAPHIC)){
					...
					if (demoName.equalsIgnoreCase("ETHNICITY") && this.isLasLinksCustomer){
						if (demoLabelName.equalsIgnoreCase("HISPANIC OR LATINO")){
							...
							System.out.println("**Inside getStudentDemographicData : ETHNICITY && HISPANIC OR LATINO.*OLD UI*");
					...
					if (!subEthnicityToBePresent && demoName.equalsIgnoreCase("SUB_ETHNICITY") && this.isLasLinksCustomer){
						...
						System.out.println("**Inside getStudentDemographicData : Sub_ETHNICITY && ethnicity not present.*OLD UI*");
					...
					if (demoName.equalsIgnoreCase("SUB_ETHNICITY") && this.isLasLinksCustomer && subEthnicityToBePresent ){
						...
						System.out.println("**Inside getStudentDemographicData : Sub_ETHNICITY && ethnicity present.*OLD UI*");
					...
					if (!demoName.equalsIgnoreCase("ETHNICITY") && this.isLasLinksCustomer &&  subEthnicityToBePresent){
						...
						System.out.println("**Inside getStudentDemographicData : other than sub_ethnicity && sub_ethnicity to be present.*OLD UI*"); 
						subEthnicityToBePresent = false; 
		...
		if(subEthnicityNotPresent || (count == 1)){
			System.out.println("** Ethnicity demographic check.. ReEntry in loop as Sub-Etnicity not present *OLD UI*");
			for ( int i= 0 ; i < 1 ; i++ ){
				StudentDemographic studentDemographic 
				= new StudentDemographic();
...
//		}
	...
	throws SQLException,CTBBusinessException {
		...
		try {
			...
			Node []nodeCategory = this.studentFileRowHeader[0].getOrganizationNodes();
			 // For MQC 67720: MDR columns needs to be removed for nonLaslinks
			int orgHeaderLastPosition = nodeCategory.length * orgPosFact;    	// For MQC 66840 : Upload/Download user/student with MDR
			...
			for ( int i = 1; i < totalRows; i++ ) {
				...
				if ( !(requiredMap.containsKey(new Integer(i)) 
					...
					String leafName="";
					// For MQC 67720: MDR columns needs to be removed for nonLaslinks
					for ( int ii = loginUserOrgPosition + orgPosFact; ii < orgHeaderLastPosition; ii = ii + orgPosFact ) { 	// For MQC 66840 : Upload/Download user/student with MDR
	...
	private boolean hasOrganization (int currentPosition, HSSFRow row) {
		...
		String leafOrgName = "";
		 // For MQC 67720: MDR columns needs to be removed for nonLaslinks
		for (int j = currentPosition + orgPosFact ; j < OrgHeaderLastPosition; j = j + orgPosFact ) { 	// For MQC 66840 : Upload/Download user/student with MDR
			HSSFCell cellName = row.getCell(j);
			HSSFCell cellId = row.getCell(j + 1);
