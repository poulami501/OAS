...
{ 
	...
	@org.apache.beehive.controls.api.bean.Control()
	private com.ctb.control.db.LoadTestDB loadTest;
	...
	public AdssvcResponseDocument getSubtest(AdssvcRequestDocument document)
	{
		...
		boolean blockDownloadFlag = false;
		int tdSubtestId=0;
		int tcSubtestId=0;


		try {
			// change for load test to block the content download
			tdSubtestId = loadTest.getTDItemSetId(subtestId);
			tcSubtestId = loadTest.getTCItemSetId(tdSubtestId);
			blockDownloadFlag = (loadTest.getBlockFlag(tcSubtestId) != null && loadTest.getBlockFlag(tcSubtestId).equals("T")? true : false);
			System.out.println("blockDownloadFlag..." + blockDownloadFlag);
			
			if (!blockDownloadFlag) { // change for load test to block the content download
			...
			} else { // change for load test to block the content download
				throw new Exception("Subtest with subtest id '"+subtestId+"' is blocked");
			}
		}
		catch (Exception e)
		{
