...
{ 
    ...
    public static void setPermissionRequestAttributeORG(HttpServletRequest request, 
                                                        ManageOrganizationForm orgForm, boolean isEditPage) {
        String actionPermission = DEFAULT_PERMISSION_TOKEN;  
        
        if (isEditPage) {
            
            actionPermission = orgForm.getActionPermission();
            
        } else {
            
            if ( orgForm.getSelectedOrgNodeId() != null ) {
        ...
        request.setAttribute("disableADDButton", isADDDisabled(actionPermission));
        
    }
    
     /**
     * setPermissionRequestAttributeFile
     */    
    public static void setPermissionRequestAttributeFile(HttpServletRequest request, 
                                                         ManageUploadForm fileForm){
        
        String actionPermission = DEFAULT_FILE_PERMISSION_TOKEN;
        
        if ( fileForm.getActionPermission() != null) {
            
            actionPermission = fileForm.getActionPermission();
        
        }
        
        request.setAttribute("disableDeleteButton", 
                                    isDeleteFileDisabled(actionPermission));
        request.setAttribute("disableFailedRecordButton", 
                                    isExportErrorDisabled(actionPermission));
        request.setAttribute("disableRefreshButton",
                                    isRefreshDisabled(actionPermission));
