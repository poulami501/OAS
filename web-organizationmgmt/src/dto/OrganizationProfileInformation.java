...
public class OrganizationProfileInformation extends SanitizedFormField{
    ...
    public OrganizationProfileInformation(Node node) {
    	
    	this.orgNodeId = node.getOrgNodeId();
    	this.customerId = node.getCustomerId();
    	this.orgNodeCategoryId = node.getOrgNodeCategoryId();
    	this.orgNodeName = node.getOrgNodeName();
    	this.extQedPin = node.getExtQedPin();
    	this.extElmId = node.getExtElmId();
    	this.extOrgNodeType = node.getExtOrgNodeType();
    	this.orgNodeDescription = node.getOrgNodeDescription();
    	this.createdBy = node.getCreatedBy();
    	this.createdDateTime = node.getCreatedDateTime();
    	this.updatedBy = node.getUpdatedBy();
    	this.updatedDateTime = node.getUpdatedDateTime();
    	this.activationStatus = node.getActivationStatus();
    	this.dataImportHistoryId = node.getDataImportHistoryId();
    	this.parentState = node.getParentState();
    	this.parentRegion = node.getParentRegion();
    	this.parentCounty = node.getParentCounty();
    	this.parentDistrict = node.getParentDistrict();
    	this.orgNodeCode = node.getOrgNodeCode();
    	this.orgNodeCategoryName = node.getOrgNodeCategoryName();
    	this.childNodeCount = node.getChildNodeCount();
    	this.parentOrgNodeId = node.getParentOrgNodeId();
    	this.parentOrgNodeName = node.getParentOrgNodeName();
    	this.orgNodeCategory = node.getOrgNodeCategory();
    	this.editable = node.getEditable();
    	this.categoryLevel = node.getCategoryLevel();
    	this.mdrNumber = node.getMdrNumber();
    	this.leafNodePath = node.getLeafNodePath();
