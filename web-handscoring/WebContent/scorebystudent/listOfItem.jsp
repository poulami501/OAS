...
<netui-template:template templatePage="/resources/jsp/scoring_template.jsp">
 ...
 %>
							...
							<td class="sortable">
							<a href="javascript:void(0)" onclick="openViewQuestionWindow('<%=itemId%>','<%=itemNumber%>'); return true;">View Question</a></td>
 ...
 %>
							...
							<netui-data:getData resultId="answered" value="${container.item.answered}" /> <c:if test="${itemtype =='AI'}">
								<c:if test="${answered == 'NA'}">
									<span><font color="#999999"><u>Audio Response</u></font></span>
								</c:if>
							...
							<c:if test="${itemtype =='CR'}">
								<c:if test="${answered == 'NA'}">
									<span><font color="#999999"><u>Text Response</u></font></span>
								</c:if>
								<c:if test="${answered == 'A'}">
