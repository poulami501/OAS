...
class PositionalRecordParser extends BaseRecordParser implements RecordParser {
	...
	public <T> String parseToText(T record) throws RecordParserException {
		...
		for (int i = 0; i < positionalFieldDescriptors.size(); i++) {
			...
			String sizedFieldValue = StringUtil.fillToLength(fieldValue, fieldLength, actualFieldDescriptor.getPaddingCharacter(), StringUtil.Direction.valueOf(actualFieldDescriptor.getPaddingAlign().toString()),StringUtil.Direction.valueOf(actualFieldDescriptor.getJustify().toString()));
