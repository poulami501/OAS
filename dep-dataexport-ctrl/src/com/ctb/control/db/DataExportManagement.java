...
{ 
   ...
   */
    ...
    @JdbcControl.SQL(statement = " SELECT testAdminId, testSessionName, startDate, endDate, to_char(startDate, 'MM/DD/YY') as startDateString, to_char(endDate, 'MM/DD/YY') as endDateString, status, dailyLoginStartTime, dailyLoginEndTime, timeZone FROM (select DISTINCT tadmin.test_admin_id as testAdminId, tadmin.test_admin_name as testSessionName, tadmin.login_start_date as startDate, tadmin.login_end_date as endDate, tadmin.test_admin_status as status, tadmin.daily_login_start_time as dailyLoginStartTime, tadmin.daily_login_end_time as dailyLoginEndTime, tadmin.time_zone as timeZone from test_admin tadmin, program prog, users u, user_role ur, product prod, org_node_ancestor ona  where tadmin.login_start_date BETWEEN prog.program_start_date and prog.program_end_date and tadmin.login_end_date BETWEEN prog.program_start_date and prog.program_end_date and tadmin.program_id = prog.program_id and prog.activation_status = 'AC' and prog.customer_id = {customerId} and tadmin.creator_org_node_id = ona.org_node_id  and ona.ancestor_org_node_id = ur.org_node_id and ur.user_id = u.user_id and prod.product_id = tadmin.product_id and prod.parent_product_id = {productId} and prod.product_type <> 'ST' and u.user_name = {userName}  ORDER BY tadmin.test_admin_name)" , arrayMaxLength=0, fetchSize = 50000)
    ManageTestSession[] getTestSessionForExportWithStudentsForUserWithFrameworkId(Integer customerId, String userName, Integer productId)throws SQLException;
