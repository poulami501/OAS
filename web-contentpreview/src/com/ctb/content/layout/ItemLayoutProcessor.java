...
//    public static int tableTitleFontSize = 14; 
    ...
    public ItemLayoutProcessor( Element itemElement_, String MaxPanelWidth, int order_, int pointSize_, boolean addCData_, List unicodeList_, boolean 
    ...
    public void reDoScrollWidgetTextWidthHeight()throws Exception
    {
        ...
        if ( stimulus != null )
        {
            ...
            for ( int j = 0; j < passage_widgetList.size(); j++ )
            {
                ...
                {
                    Element theWidget = ( Element )textWidgetList.get( i );
                    if (!"cell".equals(theWidget.getParent().getName())) {
        ...
        if ( children.size() == 2 )
        {
            ...
            if ( firstBorn.getName().equals( image_widget ) )
            {
                ...
                if ( lastBorn.getName().equals( text_widget ) )
                {
                    ...
                    lastBorn.setAttribute( "width", String.valueOf( panelWidth - left_padding - panelSpacing - imageWidth - scrollbarWidth - 
text_to_scrollbar_margin ));
                }
            }
        }
    }
...
//            }
            ...
            for ( i = 0; i < content.size(); i++ )
            {
                ...
                else
                {
                    ...
                    if ( (text_widget.equals( widget.getName() ) || "text".equals( widget.getName() ) )
                        && !"cell".equals(widget.getParent().getName()))
                        widget.setAttribute( "width", String.valueOf( widgetMaxWidthAllowed ));
                }
            }
    ...
    public Font getFont( boolean developement ) throws Exception
    {
        if ( ctbFont == null )
        {
            Font newFont = Font.createFont( Font.TRUETYPE_FONT, this.getClass().getResourceAsStream("/resources/fonts/OASmathv3.ttf"));
    ...
    public Font getBoldFont() throws Exception
    {
        if ( ctbBoldFont == null )
        {
            Font newFont = Font.createFont( Font.TRUETYPE_FONT, this.getClass().getResourceAsStream("/resources/fonts/OASmathv3 Bold.ttf"));
...
//        FileInputStream fis = new FileInputStream( new File( srcFile ) );
    ...
    public void getProcessableChildInOrder( Element element, List childSrcList)throws Exception
    {
            ...
            if ( thisObject instanceof Element )
            {
                ...
                else if ( name.equals( "Flash" ) )
                {
                    ...
                    if ( bInTextElement )
                    {
                        ...
                        Element parent = thisElement.getParent();
                 ...
                 //SM   srcImage = "http://mcsdoas15.mhe.mhc:9000/images/" + srcImage.substring(srcImage.indexOf("/images/") + 8).replaceAll(" ", "%20");
        //            String tempImage = copyThisImage( srcImage );
       ...
       //             widgetElement.setAttribute( "src", tempImage );
                    String altText = getAttributeValue( thisElement.getParent(), "Alt" );
                    if ( altText != null )
                ...
                else if ( name.equals( "Passage" ))
                {
                    ...
                    if ( tabStimulus )
                    {
                        ...
                        if (newID == null) { // if ID not in Passage, then get it from parent stimulus
                            Element stimulus = (Element) thisElement.getParent();
                    ...
                    while (it.hasNext()) {
                        ...
                        String altText = srcFootnote.getParent().getAttributeValue("AltText");
      ...
      //                  boundingElement.setAttribute( "multiline", "true" );
                        ...
                        if (!isInlineImage) {
                            ...
                            CDATA cdata = (CDATA)Cdata.get(0);
                            String textCada = cdata.getText();
                            //String textCada = (String)Cdata.get(0);
...
*/
                    ...
                    String altText = thisElement.getParent().getAttributeValue("AltText");
                ...
                else if ( name.equals( "CRResponseArea" ) )
                {
                    ...
                    if (!isMultiLine && "Text".equals(thisElement.getParent().getName()))
                        inline = true;
            ...
            else if ( thisObject instanceof Text ) 
            {
                ...
                if(isInlineImage){
                	if (childSrcList.size() != 0 ) {
                        ...
                        String textCada = cdata.getText();
                        //add inline_image to CDATA
                        if(thisText.getValue() != null && thisText.getValue().length()>0){
                        	textCada = textCada.concat(" "+thisText.getValue()+" ");
                        }
                        //remove existing CDATA
            ...
            else // right_column
            {
                ...
                if ( widthS == null )
                {
                    if ( stimulus != null )
                    {
                        ...
                        int width = LAYOUT_MAX_WIDTH - ( Integer.valueOf( stimulusX ).intValue() + Integer.valueOf( stimulusWidth ).intValue() ) - 
rightIndent;
                        stem.setAttribute( "width", String.valueOf( width ) );
                    }
                    else
...
//        item_model.setAttribute( "iid", itemElement.getAttributeValue( "ID" ) );
    ...
    public static void getAsset( Element layoutItem, List assetList ) throws Exception
    {
        ...
        for ( int i = 0; i < localAssetList.size(); i++ ) 
        {
            ...
            List Cdata = text_widget.getContent();
            CDATA cdata = (CDATA)Cdata.get(0);
    ...
    public static void modifyItemLMLForADS_Puslishing( Element itemLML , String itemType) throws Exception
    {
        ...
        for ( int i = 0; i < imageList.size(); i++ ) 
        {
        	...
        	if(textContent.indexOf("image_widget") >= 0) {
                ...
                text_widget.removeContent(cdata);
        		text_widget.addContent(new CDATA(textContent));
        	}
        }
