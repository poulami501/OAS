 ...
 (function( $ ){
	...
	$.sliderTabs = function(container, options){
		...
		plugin.init = function(){
			...
			// Tab arrows
			$leftTabArrow = $("<a href='#' class='ui-slider-left-arrow'><img class='left_arrow_img' src='/SessionWeb/resources/css/images/ui-sliderleftarrow.png'/></a>").css({
			...
			$rightTabArrow = $("<a href='#' class='ui-slider-right-arrow'><img class='right_arrow_img' src='/SessionWeb/resources/css/images/ui-sliderrightarrow.png'/></a>").css({
				'width': settings.tabArrowWidth,
				'height': settings.tabHeight+2
			}).appendTo($tabsListContainer).click(function(e){
		...
		plugin.selectTab = function(tab){
			...
			var targetId = ($clicked.find('a').attr('targetID')).substr(1);
			...
			if(settings.transition == 'slide')
				...
				$tabsList.children('li').each(function(index, el){
					var selectedIndex = $tabsList.children('.selected').index(),
						thisIndex = $(el).index();
					var panel = $contentDivsContainer.children('#'+$(el).find('a').attr('targetID').substr(1));
