	...
	if (isHidden) {
		$.ajax(
		{
				...
				success:	function(data, textStatus, XMLHttpRequest){
								...
								$('#myProfileDialog').bind('keypress', function(event) {
	...
	$.ajax(
		...
		success:	function(data, textStatus, XMLHttpRequest){	
						...
						if(successFlag) {
							...
							$("#saveTestTitle").text(data.content);
							$("#saveTestTitle").show();
							$('#errorIcon').hide();
							$('#infoIcon').show();
							document.getElementById('showSaveTestMessage').style.display = "block";
							//orgNodeIds = "";							
						}
  						else{
  							setMessageProfile(data.title, data.content, data.type, "");    
  							$("#saveTestTitle").html(data.content);
							$("#saveTestTitle").show();
							$('#errorIcon').show();
							$('#infoIcon').hide();
  							document.getElementById('displayMessageMyProfile').style.display = "block";					
