    ...
    function pushInsideElement(currentNodeId,currentCategoryLevel,dataObj2,currentTreeArray){
	  ...
	  function streamInnerPush(objArray,ulElement,fragment){
		...
		if (objArray.attr.cid == leafNodeCategoryId){
			var isWVStateUser = $("#addStudentEnable").val();
			var isWVCustomer = $("#isWVCustomer").val();
			if(isWVCustomer == 'true')
			{
				if( isWVStateUser == 'true')
				{
				...
				{
					liElement.innerHTML = "<ins class=\"jstree-icon\">&nbsp;</ins><ins class=\"jstree-icon\">&nbsp;</ins>" + objArray.data + "</a> ";
				}
			}
			else
			{
  ...
  function openNextLevel(asyncOver){
	...
	function populateStuDemographics(){
		...
		for(var count=0; count< stuDemographic.length; count++) {
			...
			if(stuDemographic[count]['studentDemographicValues'].length == 1){
		     	...
		     	if(document.getElementById(dynKey) != undefined && document.getElementById(dynKey) != null){
			     	if(trim(stuDemographic[count]['studentDemographicValues'][0]['selectedFlag']) == 'true'){
				     	$("#Student_Additional_Information :checkbox[name='" + dynKey+ "']").attr('checked', true);	
				     	if(demoGroup != ""){
	...
	function adjustRadioSelection(value){
	    ...
	    for(var i=0; i < inputs.length; i++) {
	     ...
	     for(var counter = 0; counter < demographicGroupIds.length;counter++) {
	     var inputs= $('#'+demographicGroupIds[counter] +' input');
	    for(var i=0; i < inputs.length; i++) {
	        if(inputs[i].getAttribute("name") != undefined && inputs[i].getAttribute("name").indexOf('Sub_Ethnicity_')== 0 && inputs[i].getAttribute("type") == "radio" &&
	        	value != inputs[i].getAttribute("name")){	        
	          inputs[i].checked=false;
